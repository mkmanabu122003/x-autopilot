...
const FACT_CHECK_RULES = `## ファクトチェックルール（生成後に必ず自己検証すること）

生成した各ツイートについて、以下の観点で事実確認を行い、結果をfact_checkフィールドに記載すること。

### チェック項目
1. **数字の整合性** — ツアー回数（500回以上）、TOEIC（950）、レビュー（4.86）等のプロフィール数値と矛盾していないか
2. **地名・施設名** — 実在する地名・施設名か。架空の場所を書いていないか（例：「浅草の○○寺」が実在するか）
3. **文化・歴史的事実** — 日本文化や歴史に関する記述が正確か（寺社の名称、祭りの時期、歴史的事実等）
4. **エピソードの一貫性** — とっけんの立場（英語ガイド、東京中心、VIP対応）と矛盾するエピソードがないか
5. **常識的な妥当性** — 金額、時間、距離などの数字が常識的に妥当か

### fact_checkフィールドの記載方法
- "ok": すべてのチェックをパスした場合
- 問題がある場合: 具体的な指摘を記載（例: "「浅草の金龍寺」は架空の寺名。正しくは「浅草寺（金龍山）」"）

注意: 少しでも事実に自信がない場合は正直に指摘すること。ツッコミが入るリスクを最小化するのが目的。`;
...
- O-F: 問いかけ — 読者に直接質問を投げかけて始める（例：「ツアー中、ゲストが一番テンション上がる瞬間って何だと思う？」）リプライを誘発する
- O-G: If構文（仮定提示） — 「もし〜だったら」で読者を自分ごと化させる（例：「もし明日、英語ゼロのまま外国人を案内することになったら。」）
- O-H: 独り言・呟き — 現場の温度感をそのまま出す（例：「今日のゲスト、寺に興味ないの開始3分でわかった。」）
- O-I: 結論先出し — 最も伝えたい1文を冒頭に置く（例：「ガイドの仕事の8割は『聞くこと』だ。」）
- O-J: 時制ジャンプ — 「今」と「過去」の落差で好奇心ギャップを作る（例：「3年前の俺がこのツアーを見たら、別人だと思うだろう。」）
...
- D-E: 感情の実況 — その場の感情を時系列で描写する（焦り→発見→安堵など）。読者が追体験できる
- D-F: 数字の階段 — 具体数字の積み重ねで変化を見せる（「1回目→50回目→500回目」）
- D-G: 会話の再現 — ゲストと自分のやり取りを2〜3往復再現する。画像なしで「動画的」な没入感
- D-H: 裏側暴露 — 業界の常識や本音をぶっちゃける。「知られざるリアル」は保存されやすい
...
- C-E: 自分への問い — 内省で終わる（例：「俺はまだ、ちゃんと見えているだろうか。」）保存されやすい
- C-F: 未来予測 — 「次はこうなる」で読者の期待を作る（例：「来年、この業界は激変する。」）
- C-G: 一文回収 — 冒頭のフックを最後に回収して構造的快感を与える（冒頭「嘘だ」→ 締め「あの日から嘘じゃなくなった。」）
- C-H: 沈黙・省略 — 言い切らずに体言止めや省略で読者に委ねる（例：「500回目のツアー。まだ、知らないことだらけだ。」）`;

const JSON_OUTPUT_INSTRUCTION_TWEET = `出力はJSON形式で返してください。コードフェンス（\`\`\`json）は付けないこと：
{"variants":[{"label":"この案の狙い（1行）","body":"投稿本文","char_count":文字数,"opening_pattern":"O-X","development_pattern":"D-X","closing_pattern":"C-X","expressions":["特徴的な表現1","特徴的な表現2"],"fact_check":"ok または指摘事項"}]}

- opening_pattern: 使用した冒頭パターンのコード（O-A〜O-J）
- development_pattern: 使用した展開パターンのコード（D-A〜D-H）
- closing_pattern: 使用した締めパターンのコード（C-A〜C-H）
- expressions: そのツイートで使った特徴的な表現やフレーズ（2〜3個）
- fact_check: ファクトチェック結果（"ok" または具体的な指摘事項）`;
