...
class ClaudeProvider extends AIProvider {
  async generateTweets(theme, options = {}) {
    ...
    const systemPrompt = await this.getSystemPrompt({ ...options, theme });
    ...
    await logApiUsage('claude', 'POST /v1/messages', 0.001, options.accountId);
    return { provider: 'claude', model, candidates: this.parseCandidates(data.content[0].text) };
  }
}
...
class GeminiProvider extends AIProvider {
  async generateTweets(theme, options = {}) {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ contents: [{ parts: [{ text: fullPrompt }] }], generationConfig: { maxOutputTokens: 1024 } })
    });
    ...
    const data = await response.json();
    await logApiUsage('gemini', `POST /models/${model}:generateContent`, 0.0002, options.accountId);
    return { provider: 'gemini', model, candidates: this.parseCandidates(data.candidates[0].content.parts[0].text) };
  }
}
